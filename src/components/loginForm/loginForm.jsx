...
const clientId = "Ваш_клиентский_идентификатор"; // Замените на свой клиентский идентификатор
...
const LoginForm = () => {
  ...
  const handleGoogleSuccess = (response) => {
    console.log("Успешная аутентификация через Google:", response);
    // Здесь вы можете выполнить необходимые действия после успешной аутентификации через Google
  };

  const handleGoogleFailure = (error) => {
    console.error("Ошибка аутентификации через Google:", error);
    // Здесь вы можете обработать ошибку аутентификации через Google
  };
        ...
        <div className="login">
          ...
          <div className="login--n1">
            ...
            >
              ...
              <h2
                style={{ color: none ? "red" : "", transition: "0.8s" }}
                onClick={() => {
                  toggleMode();
                  setTimeout(() => {
                    setReverse(false);
          ...
          <div className="login--cnt">
            <div
              ...
              className="login--cnt__images"
            ></div>
            <div
              style={{ marginLeft: `${hello}%`, transition: "0.6s" }}
              className="login--cnt__c1"
            >
              <h1 style={{ transition: "1s" }}>
                {none ? "Войти" : "Регистрация"}
              </h1>
              <p>
                {none
                  ? "Заполните данные для того что-бы Войти"
              ...
              />
              <div
                style={{ marginTop: none ? "242px" : "" }}
                className="login--cnt__c1--btn"
              >
                ...
                <button
                  ...
                  className="login--cnt__c1--btn__btns"
                >
